...
#     model.fit(xx, yy, epochs=ep, verbose=0)

#     # Evaluation on test set
#     test_loss, test_acc = model.evaluate(xt, yt, verbose=0)
#     tr_loss, tr_acc = model.evaluate(xx, yy, verbose=0)

#     ypr = model.predict(xt)
#     ypr = (ypr > 0.5).astype(int)
...
#     return tr_acc, test_acc, precision, recall, f1score
...
for i in range(30):
    ...
    train_accuracy_SMOTEGAN.append(MCMC[0])
    test_accuracy_SMOTEGAN.append(MCMC[1])
    precision_SMOTEGAN.append(MCMC[2])
    recall_SMOTEGAN.append(MCMC[3])
    f1_score_SMOTEGAN.append(MCMC[4]) 
    ypr_MCMC.append(MCMC[5])


# After collecting all precision and recall values for MCMC
precision_MCMC_avg = np.mean(precision_SMOTEGAN)
recall_MCMC_avg = np.mean(recall_SMOTEGAN)
...
with open('PAGEBLOCK_SMOTifiedGAN.pkl', 'wb') as f:
    pickle.dump((precision_MCMC_curve, recall_MCMC_curve, pr_auc_MCMC), f)

print("Smotified Train - ",np.mean(train_accuracy_SMOTEGAN))
print("Smotified Test - ",np.mean(test_accuracy_SMOTEGAN))
print("Smotified F1-score - ",np.mean(f1_score_SMOTEGAN))
print("Smotified precision - ",np.mean(precision_SMOTEGAN))
print("Smotified recall - ",np.mean(recall_SMOTEGAN))


print("Smotified Train - ",train_accuracy_SMOTEGAN)
print("Smotified Test - ",test_accuracy_SMOTEGAN)
print("Smotified F1-score - ",f1_score_SMOTEGAN)
print("Smotified precision - ",precision_SMOTEGAN)
print("Smotified recall - ",recall_SMOTEGAN)