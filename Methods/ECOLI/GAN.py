...
def callf1(xx, yy, xt, yt, ep):
    ...
    y_pred = F1_model.predict(xt).flatten()
    ypr = (y_pred > 0.5).astype(int)
    ...
    return tr_acc, test_acc, precision, recall, f1score, y_pred
...
for epoch in range(numberof_epochs):
    for real, _ in tqdm(dataloader):
        ...
        disc_loss_SMOTE = get_disc_loss_smote(genGAN, disc_GAN, criterion, real, fake_noise )
        disc_loss_SMOTE.backward(retain_graph=True)
        disc_opt_GAN.step()

        disc_opt_GAN.zero_grad()
        gen_loss_SMOTE = get_gen_loss_smote(genGAN, disc_GAN, criterion, fake_noise)
...
generated_data_SMOTE = genGAN(fake_noise)
...
with open('results_GAN.pkl', 'wb') as f:
    pickle.dump((precision_MCMC_curve, recall_MCMC_curve, pr_auc_MCMC), f)

print("GAN Train - ",np.mean(train_accuracy_MCMC))
print("GAN Test - ",np.mean(test_accuracy_MCMC))
print("GAN F1-score - ",np.mean(f1_score_MCMC))
print("GAN precision - ",np.mean(precision_MCMC))
print("GAN recall - ",np.mean(recall_MCMC))


print("GAN Train - ",train_accuracy_MCMC)
print("GAN Test - ",test_accuracy_MCMC)
print("GAN F1-score - ",f1_score_MCMC)
print("GAN precision - ",precision_MCMC)
print("GAN recall - ",recall_MCMC)


## BELOW OS FOR THE DIAGRAM 

# train, test,precision_smote, recall_smote, f1_smote, y_pred_smote = callf1(XSmotified,ySmotified,X_test,y_test, ep=30)

# precision_smote_curve, recall_smote_curve, _ = precision_recall_curve(y_test, y_pred_smote)
# auc_smote = auc(recall_smote_curve, precision_smote_curve)

# print(f'SMOTE PR AUC: {auc_smote:.3f}')
# print('recall_smote_curve',recall_smote_curve)
# print('precision_smote_curve',precision_smote_curve) 


# plt.figure(figsize=(10, 8))
# plt.plot(recall_smote_curve, precision_smote_curve, label=f'GAN (AUC={auc_smote:.2f})')
# plt.xlabel('Recall')
